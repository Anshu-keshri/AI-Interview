...
function Auth({isModel = false}) {
    ...
    const handleGoogleAuth = async() => {
        try{
            ...
            console.log('Backend /api/auth/google response:', result)
            dispatch(setUserData(result.data))
        }
        catch(error){
            console.error('Google auth error:', error)
            // show a visible error for debugging
            try{ alert(error?.message || JSON.stringify(error)) } catch(e){}
  ...
  return (
    <div className={`w-full ${isModel ? "py-4" : "min-h-screen bg-[#f3f3f3] flex items-center justify-center px-6 py-20"}`}>
        ...
        className={`w-full ${isModel ? "max-w-md px-8 py-6 rounded-3xl" : "max-w-lg p-12 rounded-[32px]"} bg-white shadow-2xl border border-gray-200`}>
